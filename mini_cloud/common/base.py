...
from mini_cloud.utils.py3 import ET

import requests
import mini_cloud

from mini_cloud.utils.py3 import httplib
from mini_cloud.utils.py3 import urlparse
from mini_cloud.utils.py3 import urlencode

from mini_cloud.utils.misc import lowercase_keys, retry
from mini_cloud.common.exceptions import exception_from_message
from mini_cloud.common.types import LibcloudError, MalformedResponseError
from mini_cloud.http import MiniCloudConnection, HttpLibResponseProxy
import gevent
...
class Connection(object):
    ...
    """
    conn_class = MiniCloudConnection
    ...
    def _user_agent(self):
        ...
        if self.driver:
            user_agent = 'mini_cloud/%s (%s) %s' % (
                mini_cloud.__version__,
                self.driver.name, user_agent_suffix)
        else:
            user_agent = 'mini_cloud/%s %s' % (
                mini_cloud.__version__, user_agent_suffix)
    ...
    def async_request(self, action, params=None, data=None, headers=None,
        ...
        self.connection.async_request(method,url,bo)

    # def async_request(self, action, params=None, data=None, headers=None,
    #                   method='GET', raw=False, stream=False):
    #     return gevent.spawn_later(0, self.request, *(action, params, data, headers, method, raw, stream))
