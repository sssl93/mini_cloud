...
class ECSNodeDriver(ECSDriver):
    ...
    SECURITY_RULE_KEYS = ['group_id', 'location', 'direction', 'protocol',
                          'port_range', 'dest_cidr', 'source_cidr']
    ...
    def _to_security_rule(self, element, group_id, location):
        ...
        cidr_key = 'DestCidrIp' if direction == 'egress' else 'SourceCidrIp'
        base_data = {
            'group_id': group_id, 'location': location, 'direction': direction,
            'cidr': findtext(element, cidr_key, namespace=self.namespace),
            'dest_cidr': findtext(element, 'DestCidrIp', namespace=self.namespace),
            'source_cidr': findtext(element, 'SourceCidrIp', namespace=self.namespace),
    ...
    def ex_create_security_rule(self, group_id, location, name, direction, policy, cidr,
        ...
        params = {'Action': action, 'RegionId': location, 'SecurityGroupId': group_id,
                  ...
                  'Policy': policy, 'Priority': priority, 'Description': name}

        if direction == 'ingress':
            params['SourceCidrIp'] = cidr or '0.0.0.0/0'
        elif direction == 'egress':
            params['DestCidrIp'] = cidr or '0.0.0.0/0'
        else:
            raise Exception("Invalid direction `%s`, 'ingress' and 'egress' were supported." % (direction,))
    ...
    def ex_delete_security_rule(self, rule_id):
        ...
        params = {'Action': action, 'RegionId': source['location'], 'SecurityGroupId': source['group_id'],
                  ...
                  'SourceCidrIp': source['source_cidr'], 'DestCidrIp': source['dest_cidr']}
