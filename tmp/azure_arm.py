...
from libcloud.compute.drivers.azure_arm import AzureNetworkSecurityGroup, AzureImage, RESOURCE_API_VERSION, AzureSubnet, \
    AzureVhdImage, NodeAuthSSHKey, NodeAuthPassword
...
class AzureNodeDriver(azure_arm.AzureNodeDriver):
    ...
    def _to_security_rule(rule, group_id):
        ...
        port_rage = ','.join(properties['destinationPortRanges'])
        if not port_rage:
            port_rage = properties['destinationPortRange']

        cidr = ','.join(properties['destinationAddressPrefixes'])
        if not cidr:
            cidr = properties['destinationAddressPrefix']
        data = {'id': rule['id'], 'base64_id': base64.b64encode(rule['id']), 'name': rule['name'],
                ...
                'port_range': port_rage, 'policy': policy_map.get(properties['access']),
                'cidr': cidr, 'group_id': group_id}
